...
test("classifyExecResult: exit 0 with no question -> {exitCode:0, signal:null}, no 'question' key", () => {
  const r = mod.classifyExecResult({ status: 0, signal: null, stdout: "ok", stderr: "" });
...
test("classifyExecResult: signal present is preserved verbatim (no silent loss)", () => {
  const r = mod.classifyExecResult({ status: null, signal: "SIGTERM", stdout: "", stderr: "" });
  assert.equal(r.signal, "SIGTERM");
});

test("classifyExecResult: question_packet in stdout takes priority, exitCode/signal still attached", () => {
  const r = mod.classifyExecResult({ status: 0, signal: null, stdout: 'question_packet:\n  question_id: "Q-1"\n', stderr: "" });
  assert.deepEqual(r, { exitCode: 0, signal: null, question: { question_id: "Q-1" } });
});
...
test("classifyExecResult: malformed raw (undefined/null) is fail-closed to null exitCode, never throws", () => {
  ...
  assert.deepEqual(mod.classifyExecResult(undefined), { exitCode: null, signal: null });
...
test("createClaudeAdapterFn: invokes spawnSyncFn with shell:false, array args, explicit-only env (no process.env passthrough)", () => {
  ...
  const adapterFn = mod.createClaudeAdapterFn({ command: "claude", spawnSyncFn });
  const r = adapterFn({ task_id: "T-1", attempt_id: "A-1", cwd: "C:/work", lane: "coder", config: "p.json", at: "t" });
  assert.equal(r.exitCode, 0);
  assert.equal(captured.command, "claude");
  assert.ok(Array.isArray(captured.args), "args must be a plain array (no shell string concatenation)");
  assert.ok(captured.args.includes("T-1"));
  assert.ok(captured.args.includes("A-1"));
  assert.equal(captured.options.shell, false, "shell must be explicitly false -- no shell interpretation of args");
  assert.equal(captured.options.cwd, "C:/work");
  assert.deepEqual(captured.options.env, {}, "no config.env supplied -> child gets an explicit empty env, never process.env");
});
...
test("createClaudeAdapterFn: env is explicit-only -- only keys passed via config.env reach the child (no ambient secret leakage)", () => {
  let captured = null;
  const spawnSyncFn = (command, args, options) => { captured = options; return { status: 0, signal: null, stdout: "", stderr: "" }; };
  const adapterFn = mod.createClaudeAdapterFn({ spawnSyncFn, env: { SAFE_FLAG: "1" } });
...
test("createClaudeAdapterFn: a spawnSyncFn 'error' result (e.g. ENOENT) throws -- absorbed by existing startup_failure path, not re-handled here", () => {
  const spawnSyncFn = () => ({ error: Object.assign(new Error("spawn claude ENOENT"), { code: "ENOENT" }) });
...
test("E2E-real-process: a real spawnSync against a fake exit-0 script yields {exitCode:0, signal:null}", () => {
  ...
  try {
    ...
    const adapterFn = mod.createClaudeAdapterFn({ command: process.execPath, baseArgs: [script] });
...
test("E2E-real-process: an unreachable command (ENOENT) throws through the real spawnSync path (no fake stub)", () => {
  const dir = freshDir();
  try {
    const adapterFn = mod.createClaudeAdapterFn({ command: join(dir, "definitely-does-not-exist-binary") });
    assert.throws(() => adapterFn({ task_id: "T-1", attempt_id: "A-1", cwd: dir }));
  } finally {
    cleanup(dir);
  }
});
...
test("E2E-real-process: a hung script is killed deterministically via spawnSync timeout, signal observed (join 결정론)", () => {
  ...
  try {
    ...
    const adapterFn = mod.createClaudeAdapterFn({ command: process.execPath, baseArgs: [script], spawnSyncFn: (cmd, args, opts) => {
      return realSpawnSync(cmd, args, { ...opts, timeout: 500 });
    } });
    const r = adapterFn({ task_id: "T-1", attempt_id: "A-1", cwd: dir });
    assert.notEqual(r.signal, null, "a timeout-killed process must report a non-null signal, not silently vanish (it ran -- not a startup_failure)");
  } finally {
    cleanup(dir);
  }
});
...
test("SYNTH-E2E-1: full arm lifecycle with a real fake-engine process -- done outcome, exactly one process spawned, no gate commands observed", () => {
  ...
  try {
    ...
    const adapterFn = mod.createClaudeAdapterFn({ command: process.execPath, baseArgs: [script], spawnSyncFn });
    const task = makeE2ETask(dir);

    const r = runSupervisedAttempt(dir, arm_id, makeScope(dir), task, { nowFn: NOW_OK, adapterFn });
    ...
    assert.equal(spawnSyncFn.log.length, 1, "the real engine process must be spawned exactly once (G7)");
    ...
    for (const entry of spawnSyncFn.log) {
      ...
      for (const bad of forbidden) {
        assert.equal(joined.includes(bad), false, `runtime-observed spawn call must not reference '${bad}' (G10 runtime canary): ${joined}`);
      }
    }
  } finally {
    cleanup(dir);
...
test("SYNTH-E2E-2: question outcome -- disarmed immediately, no auto-resume, exactly one spawn", () => {
  ...
  try {
    ...
    assert.equal(disk.store.receipts.find((x) => x.event === "question").detail.question_id, "Q-20260714-01");
    assert.equal(spawnSyncFn.log.length, 1, "one question observation must not trigger any further spawn");
  } finally {
    cleanup(dir);
  }
});
...
test("SYNTH-E2E-3: abnormal-exit outcome -- exit code preserved end-to-end from real process to disk receipt", () => {
  ...
  try {
    ...
    const disk = loadStore(armStorePath(dir, arm_id));
    assert.equal(disk.store.receipts.find((x) => x.event === "cli_abnormal_exit").detail.exitCode, 7);
...
test("SYNTH-E2E-4: an ENOENT engine binary is classified startup_failure by existing group-3/4 machinery, no uncaught throw escapes runSupervisedAttempt", () => {
  ...
  try {
    seedArm(dir, arm_id);
    const adapterFn = mod.createClaudeAdapterFn({ command: join(dir, "no-such-binary") });
    const task = makeE2ETask(dir);

    let result;
    assert.doesNotThrow(() => { result = runSupervisedAttempt(dir, arm_id, makeScope(dir), task, { nowFn: NOW_OK, adapterFn }); }, "I4: an adapter throw must never escape as an uncaught exception");
...
test("G10: claude-adapter source contains no human-gate call sites (static scan, complements the runtime canary above)", () => {
  const src = readFileSync(fileURLToPath(new URL("./claude-adapter.mjs", import.meta.url)), "utf8");
...
test("G-FP-1 (review-2 regression): a prose sentence mentioning 'question_packet:' plus an unrelated question_id elsewhere is NOT detected", () => {
  ...
  assert.equal(mod.detectQuestionPacket(prose), null, "marker embedded mid-sentence + unrelated question_id must not false-positive");
});
...
test("G-FP-2: a real question_packet: block still detects correctly after the fix (no regression on the true-positive path)", () => {
  ...
  assert.deepEqual(mod.detectQuestionPacket(real), { question_id: "Q-20260714-09" });
});
...
test("G-FP-6: an indented marker (nested under something else) still scopes its own deeper-indented block correctly", () => {
  ...
  assert.deepEqual(mod.detectQuestionPacket(nested), { question_id: "Q-nested-1" });
});
...
test("G-FP-7: two markers in one document -- an unrelated marker earlier with no valid child must not suppress a later real block", () => {
  const doc = "question_packet:\nno child here\n\nquestion_packet:\n  question_id: \"Q-second-block\"\n";
  assert.deepEqual(mod.detectQuestionPacket(doc), { question_id: "Q-second-block" });
});
