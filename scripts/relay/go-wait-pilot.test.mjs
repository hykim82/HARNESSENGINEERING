...
function seedArm(dir, arm_id, grantOverrides = {}) {
  ...
  assert.equal(created.ok, true, `seedArm: grant construction must succeed -- ${JSON.stringify(created)}`);
...
const SCOPE = { lane: "coder", cwd: "C:/work/repo", config: "coder-profile.json", allowedTaskIds: ["HYK-142-pilot-1"] };
...
function seedArmAtCap(dir, arm_id, { max_starts_total, attempts_total, allowed_task_ids }) {
  const created = createArmStore(makeGrant({ arm_id, max_starts_total, max_starts_per_lane: 5, max_rejections: 5, allowed_task_ids }), { at: "t0" });
  assert.equal(created.ok, true, `seedArmAtCap: grant must be valid -- ${JSON.stringify(created)}`);
  const atCap = { ...created.store, state: STATE.ARMED, attempts_total, attempts_per_lane: { coder: attempts_total } };
  const path = armStorePath(dir, arm_id);
...
function reArmOnDisk(path) {
  ...
  assert.equal(loaded.ok, true, `reArmOnDisk: store must load -- ${loaded.reason ?? ""}`);
  const reArmed = { ...loaded.store, state: STATE.ARMED, claims: {}, disarm_cause: null, disarmed_at: null, needs_human_ack: false };
...
async function scenarioS1(tally) {
  ...
  try {
    writeFileSync(join(dirA, "coder-task.md"), "task_id: HYK-142-pilot-A\ndropped_at: 2026-07-14 21:00 KST\n", "utf8");
    writeFileSync(join(dirB, "review-task.md"), "task_id: HYK-142-pilot-B\ndropped_at: 2026-07-14 21:00 KST\n", "utf8");
    // A finishes immediately; B is still pending when the loop starts, then finishes on tick 1.
    writeFileSync(join(dirA, "coder.md"), "task_id: HYK-142-pilot-A\n\n>>> DONE: CODER @ 2026-07-14 21:05 KST\n", "utf8");

    let bTicks = 0;
    const fakeSleep = () => Promise.resolve();
    const [resA, resB] = await Promise.all([
      watchResult({ role: "coder", harnessDir: dirA, intervalS: 1, maxWaitS: 60, sleepFn: fakeSleep, nowFn: () => Date.now() }),
      (async () => {
        const r = await watchResult({
          ...
          sleepFn: async () => {
            bTicks += 1;
            if (bTicks === 1) writeFileSync(join(dirB, "review.md"), "task_id: HYK-142-pilot-B\n\n>>> DONE: REVIEW @ 2026-07-14 21:06 KST\n", "utf8");
          },
          nowFn: () => Date.now(),
        });
        return r;
    ...
    assert.equal(crossCheck.ok, false, "coder role must not resolve against review's harnessDir");
  } finally {
    cleanup(dirA);
    cleanup(dirB);
  }
...
function scenarioS2(tally) {
  ...
  try {
    ...
    const first = supervisor.runSupervisedAttempt(dir, arm_id, SCOPE, makeTask(), { nowFn: NOW_OK, adapterFn });
    ...
    const retry = supervisor.runSupervisedAttempt(dir, arm_id, SCOPE, makeTask({ attempt_id: "attempt-stale-retry" }), { nowFn: NOW_OK, adapterFn });
    assert.equal(retry.ok, false, "a DISARMED arm must refuse any further claim");
    if (adapterFn.count() !== 1) tally.duplicateSpawns += 1;
    assert.equal(adapterFn.count(), 1, "stale re-claim must not invoke the adapter a second time");
  } finally {
    cleanup(dir);
  }
}
...
function scenarioS3(tally) {
  ...
  try {
    ...
    adapterFn.impl = () => ({ exitCode: 0, signal: null });
    let writeCalls = 0;
    const crashingWriteFileFn = (...args) => {
      writeCalls += 1;
      if (writeCalls === 1) throw new Error("simulated crash: disk full writing RUNNING state");
      return undefined;
    };
    const started = startTx(dir, arm_id, { task_id: task.task_id, attempt_id: task.attempt_id, at: task.at }, {
      nowFn: NOW_OK,
      spawnFn: () => { adapterFn(); },
      writeFileFn: crashingWriteFileFn,
    });
    assert.equal(started.spawned, false, "start must fail closed when RUNNING cannot be persisted");
    if (adapterFn.count() !== 0) tally.duplicateSpawns += 1;
    assert.equal(adapterFn.count(), 0, "spawn must never be invoked before RUNNING is durably saved (I2)");
    ...
    assert.equal(disk.store.state, STATE.CLAIMED, "a failed RUNNING persist must never leave RUNNING on disk");
    ...
    const retried = startTx(dir, arm_id, { task_id: task.task_id, attempt_id: "attempt-1-retry", at: task.at }, { nowFn: NOW_OK, spawnFn: () => { adapterFn(); } });
    assert.equal(retried.spawned, false, "a mismatched attempt_id (never claimed) must be refused, not silently started");
    assert.equal(adapterFn.count(), 0, "still zero spawns -- the crashed attempt has no valid path to a phantom start");
  } finally {
    cleanup(dir);
  }
}
...
function scenarioS4(tally) {
  ...
  try {
    ...
    const adapterFn = counter();
    const r = supervisor.runSupervisedAttempt(dir, arm_id, SCOPE, makeTask(), { nowFn: NOW_OK, adapterFn });
    assert.equal(r.ok, false);
    if (adapterFn.count() !== 0) tally.duplicateSpawns += 1;
    assert.equal(adapterFn.count(), 0, "a corrupt store must never reach the adapter");
  } finally {
    cleanup(dir);
  }
}
...
function scenarioS5(tally) {
  ...
  try {
    ...
    const adapterFn = counter();
    adapterFn.impl = () => ({ exitCode: 0, signal: null, question: { question_id: "q-pilot-1" } });
    const r = supervisor.runSupervisedAttempt(dir, oldArmId, SCOPE, makeTask(), { nowFn: NOW_OK, adapterFn });
    ...
    assert.ok(qReceipt, "question outcome must leave a discoverable receipt -- a silent pause is a real defect");
    assert.equal(qReceipt.detail.question_id, "q-pilot-1");

    // The old arm must refuse any further claim (no in-place resume).
    const retryOnOld = claimTx(dir, oldArmId, makeTask({ attempt_id: "attempt-resume-attempt" }), { nowFn: NOW_OK });
    assert.equal(retryOnOld.spawnAllowed, false, "resuming a QUESTION-disarmed arm in place must be refused");

    // Resume must go through a brand-new arm (new grant, new store).
    seedArm(dir, newArmId);
    const resumed = supervisor.runSupervisedAttempt(dir, newArmId, SCOPE, makeTask({ attempt_id: "attempt-resume-2" }), { nowFn: NOW_OK, adapterFn: counter() });
    assert.equal(resumed.ok, true, "a fresh arm must be able to carry the resumed attempt");
  } finally {
    cleanup(dir);
  }
}
...
function scenarioS6(tally) {
  ...
  try {
    ...
    const started = startTx(dir, arm_id, { task_id: task.task_id, attempt_id: task.attempt_id, at: task.at }, { nowFn: NOW_OK, spawnFn: () => {} });
    assert.equal(started.spawned, true);
    const midCrash = loadStore(armStorePath(dir, arm_id));
    assert.equal(midCrash.store.state, STATE.RUNNING, "arm must be left RUNNING to simulate an orphaned crash");

    const adapterFn = counter();
    adapterFn.impl = () => ({ exitCode: 0, signal: null });
    const restarted = supervisor.runSupervisedAttempt(dir, arm_id, SCOPE, task, { nowFn: NOW_OK, adapterFn });
    assert.equal(restarted.phase, "restart_recovery");
    if (adapterFn.count() !== 0) tally.duplicateSpawns += 1;
    assert.equal(adapterFn.count(), 0, "restart recovery must never blindly re-spawn a possibly-still-running attempt");

    const disk = loadStore(armStorePath(dir, arm_id));
    assert.equal(disk.store.state, STATE.DISARMED);
    assert.equal(disk.store.disarm_cause, DISARM_CAUSE.INCOMPLETE_CLAIM_RESTART);
    assert.equal(disk.store.needs_human_ack, true, "restart recovery must require a human ack, not silently resolve itself");
  } finally {
    cleanup(dir);
  }
}
...
function scenarioS7(tally) {
  ...
  try {
    ...
    const r = supervisor.runSupervisedAttempt(dir, arm_id, SCOPE, makeTask(), { nowFn: NOW_OK, adapterFn });
    assert.equal(r.ok, true, "the supervisor call itself completes -- the *outcome* is what must reflect the failure");
    assert.notEqual(r.outcome, "done", "a non-zero exit with no question must never be recorded as done");
    ...
    assert.ok(rec, "the abnormal exit must leave a discoverable receipt, not vanish");
  } finally {
    cleanup(dir);
  }
}
...
function scenarioS8(tally) {
  ...
  try {
    ...
    const adapterFn = counter();
    const reportStatusFn = () => { throw new Error("STATUS.md locked by a concurrent editor"); };
    const r = supervisor.runSupervisedAttempt(dir, arm_id, SCOPE, makeTask(), { nowFn: NOW_OK, adapterFn, reportStatusFn });
    assert.equal(r.ok, false);
    assert.equal(r.phase, "status_report");
    if (adapterFn.count() !== 0) tally.duplicateSpawns += 1;
    assert.equal(adapterFn.count(), 0);
    const after = loadStore(path);
    assert.equal(after.store.attempts_total, before.store.attempts_total, "a refused pre-claim status report must not spend any budget");
  } finally {
    cleanup(dir);
  }
}
...
function scenarioS9(tally) {
  ...
  try {
    ...
    const tampered = { ...loadedRaw.store, grant: { ...loadedRaw.store.grant, publish_allowed: true } };
    saveStoreAtomic(path, tampered);

    const adapterFn = counter();
    const r = supervisor.runSupervisedAttempt(dir, arm_id, SCOPE, makeTask(), { nowFn: NOW_OK, adapterFn });
    assert.equal(r.ok, false, "a tampered publish_allowed:true grant must never be accepted as valid");
    if (adapterFn.count() !== 0) tally.humanGateBypass += 1;
    assert.equal(adapterFn.count(), 0, "the human-gate bypass attempt must never reach the adapter");
    ...
    assert.equal(r.claim?.store?.disarm_cause, DISARM_CAUSE.STATE_CORRUPT, "the tamper refusal's disarm_cause must be state_corrupt, not some other claim failure");
    assert.ok(typeof r.claim?.reason === "string" && r.claim.reason.includes("publish_allowed"), `the tamper refusal's reason must name the tampered field -- got '${r.claim?.reason}'`);

    const after = loadStore(path);
    if (after.ok && after.store.grant && after.store.grant.publish_allowed === true) tally.humanGateBypass += 1;
  } finally {
    cleanup(dir);
  }
}
...
function scenarioS10(tally) {
  ...
  try {
    seedArmAtCap(dir, arm_id, { max_starts_total: 1, attempts_total: 1, allowed_task_ids: ["HYK-142-pilot-1", "HYK-142-pilot-2"] });
    const adapterFn = counter();
    adapterFn.impl = () => ({ exitCode: 0, signal: null });
    ...
    const r = supervisor.runSupervisedAttempt(dir, arm_id, { ...SCOPE, allowedTaskIds: ["HYK-142-pilot-1", "HYK-142-pilot-2"] }, makeTask({ task_id: "HYK-142-pilot-2", attempt_id: "attempt-2" }), { nowFn: NOW_OK, adapterFn });
    assert.equal(r.ok, false, "a claim beyond max_starts_total must be refused");
    assert.equal(r.phase, "claim", "the refusal must be the claim/budget stage, not a terminal-state artifact");
    if (adapterFn.count() !== 0) tally.budgetExceeded += 1;
    assert.equal(adapterFn.count(), 0, "no spawn past the arm cap -- a start after budget exhaustion is a real breach");
    ...
    assert.equal(r.claim?.store?.disarm_cause, DISARM_CAUSE.BUDGET_EXHAUSTED, "the refusal's disarm_cause must be the budget-exhausted cause, not some other claim failure");
    assert.ok(typeof r.claim?.reason === "string" && r.claim.reason.includes("budget exhausted"), `the refusal's reason must name budget exhaustion -- got '${r.claim?.reason}'`);

    const disk = loadStore(armStorePath(dir, arm_id));
    if (disk.ok && disk.store.attempts_total > 1) tally.budgetExceeded += 1;
    assert.ok(!disk.ok || disk.store.attempts_total <= 1, "attempts_total must never exceed the cap");
    assert.equal(disk.store.disarm_cause, DISARM_CAUSE.BUDGET_EXHAUSTED, "the on-disk disarm_cause must independently confirm the budget cause");
  } finally {
    cleanup(dir);
  }
}
...
test("PILOT DISPATCH: seed-ordered execution matches the seed's planned order, 5 criteria all 0", async () => {
  ...
  const plannedOrder = seededShuffle(REGISTRY.map((s) => s.name), PILOT_SEED);
  const executedOrder = [];
  ...
  console.log(`[go-wait-pilot] seed=${PILOT_SEED} planned_order=${plannedOrder.join(",")}`);
  console.log(`[go-wait-pilot] actual_execution_order=${executedOrder.join(",")}`);
  console.log(`[go-wait-pilot] scenarios_run=${executedOrder.length}/10`);
  console.log(`[go-wait-pilot] criteria: 중복기동=${tally.duplicateSpawns} arm상한초과=${tally.budgetExceeded} 조용한질문정지=${tally.silentQuestionPauses} 타레인소비=${tally.crossConsumption} 사람게이트우회=${tally.humanGateBypass}`);
  ...
  assert.deepEqual(executedOrder, plannedOrder, "seed-planned order must equal the order actually dispatched");
  assert.equal(executedOrder.length, 10, "all 10 registry scenarios must have been dispatched");

  assert.equal(tally.duplicateSpawns, 0, "criterion 1 (중복 기동) must be exactly 0");
  assert.equal(tally.budgetExceeded, 0, "criterion 2 (arm 상한 초과) must be exactly 0");
  assert.equal(tally.silentQuestionPauses, 0, "criterion 3 (조용한 질문 정지) must be exactly 0");
  assert.equal(tally.crossConsumption, 0, "criterion 4 (타 레인/타 설정 소비) must be exactly 0");
  assert.equal(tally.humanGateBypass, 0, "criterion 5 (사람 게이트 우회) must be exactly 0");
});
...
test("PILOT NEGATIVE CONTROLS: 5 오라클 전부 load-bearing (위반 주입 -> 탐지, 무주입 -> clean)", () => {
  ...
  // (terminal DISARMED가 아니라 예산이 결정 변수 -- review-2가 놓쳤던 바로 그 경로).
  {
    ...
    try {
      seedArmAtCap(dirClean, "arm-nc2", { max_starts_total: 1, attempts_total: 1, allowed_task_ids: ["HYK-142-pilot-1", "HYK-142-pilot-2"] });
      seedArmAtCap(dirInject, "arm-nc2", { max_starts_total: 2, attempts_total: 1, allowed_task_ids: ["HYK-142-pilot-1", "HYK-142-pilot-2"] });
      const t = makeTask({ task_id: "HYK-142-pilot-2", attempt_id: "attempt-2" });
      const clean = claimTx(dirClean, "arm-nc2", t, { nowFn: NOW_OK });
      const inject = claimTx(dirInject, "arm-nc2", t, { nowFn: NOW_OK });
      assert.equal(clean.spawnAllowed, false, "무주입(cap=1): 상한 초과 claim은 거부돼야 한다");
      assert.equal(inject.spawnAllowed, true, "주입(cap=2): 상한만 올리면 동일 claim이 admit돼야 한다 -- 안 바뀌면 상한 단언은 헛단언(S10 review-2 결함)");
      // coder-4 (review-3 게이트3): 무주입 거부가 "상한"이라는 사유로 증명되게 -- 불리언
      // 거부가 아니라 disarm_cause/reason으로 원인을 고정한다(실측: budget_exhausted).
      assert.equal(clean.store?.disarm_cause, DISARM_CAUSE.BUDGET_EXHAUSTED, "무주입 거부의 disarm_cause는 budget_exhausted여야 한다 -- cap 때문에 거부됨이 사유로 증명돼야 함");
      assert.ok(typeof clean.reason === "string" && clean.reason.includes("budget exhausted"), `무주입 거부의 reason에 budget exhausted가 포함돼야 한다 -- got '${clean.reason}'`);
      report.push(`오라클② arm상한초과: 무주입(cap=1)->REFUSED(budget_exhausted)=GREEN | 주입(cap=2)->ADMITTED (무주입 기대 'REFUSED' 적용 시 RED) ⇒ 상한 단언 load-bearing`);
    } finally {
      cleanup(dirClean);
      cleanup(dirInject);
    }
  ...
  // 증명된다(무주입 기대 'spawn=1' 적용 시 RED).
  {
    ...
    try {
      seedArm(dir, arm_id, { allowed_task_ids: ["HYK-142-pilot-1", "HYK-142-pilot-2"], max_rejections: 5 });
      const adapter = counter();
      adapter.impl = () => ({ exitCode: 0, signal: null });
      const scope2 = { ...SCOPE, allowedTaskIds: ["HYK-142-pilot-1", "HYK-142-pilot-2"] };
      const first = supervisor.runSupervisedAttempt(dir, arm_id, scope2, makeTask(), { nowFn: NOW_OK, adapterFn: adapter });
      assert.equal(first.outcome, "done");
      assert.equal(adapter.count(), 1);
      // 무주입: spent(DISARMED) arm에 다른 admissible task -> 거부, 재기동 없음.
      const cleanRetry = supervisor.runSupervisedAttempt(dir, arm_id, scope2, makeTask({ task_id: "HYK-142-pilot-2", attempt_id: "attempt-2" }), { nowFn: NOW_OK, adapterFn: adapter });
      assert.equal(cleanRetry.ok, false, "무주입: 소진된 DISARMED arm은 어떤 재드롭도 거부해야 한다");
      const countAfterClean = adapter.count();
      // 주입: terminality 유실(재-ARMED) -> 동일 재드롭이 admit -> 두 번째 spawn.
      reArmOnDisk(armStorePath(dir, arm_id));
      supervisor.runSupervisedAttempt(dir, arm_id, scope2, makeTask({ task_id: "HYK-142-pilot-2", attempt_id: "attempt-2" }), { nowFn: NOW_OK, adapterFn: adapter });
      const countAfterInject = adapter.count();
      assert.equal(countAfterClean, 1, "무주입: 소진된 arm은 다시 기동하지 않는다(spawn=1)");
      assert.equal(countAfterInject, 2, "주입: terminality가 유실되면 arm이 재기동한다 -- DISARMED 가드가 중복 기동을 막는 것임이 증명됨(안 바뀌면 헛단언)");
      report.push(`오라클① 중복기동   : 무주입(소진 arm)->REFUSED(spawns=1)=GREEN | 주입(재-ARMED)->ADMITTED(spawns=2) (무주입 기대 'spawns=1' 적용 시 RED) ⇒ 중복기동 단언 load-bearing`);
    } finally {
      cleanup(dir);
    }
  }
  ...
  // 의존함이 증명된다(무주입 기대 'found' 적용 시 RED).
  {
    ...
    try {
      seedArm(dir, arm_id);
      const adapter = counter();
      adapter.impl = () => ({ exitCode: 0, signal: null, question: { question_id: "q-nc3" } });
      const r = supervisor.runSupervisedAttempt(dir, arm_id, SCOPE, makeTask(), { nowFn: NOW_OK, adapterFn: adapter });
      assert.equal(r.outcome, "question");
      const path = armStorePath(dir, arm_id);
      const cleanDisk = loadStore(path);
      const cleanFound = !!cleanDisk.store.receipts.find((x) => x.event === "question");
      // 주입: question receipt를 벗겨 영속 -- 관측 불가능해진 silent pause.
      const stripped = { ...cleanDisk.store, receipts: cleanDisk.store.receipts.filter((x) => x.event !== "question") };
      saveStoreAtomic(path, stripped);
      const injectDisk = loadStore(path);
      const injectFound = !!injectDisk.store.receipts.find((x) => x.event === "question");
      assert.equal(cleanFound, true, "무주입: question outcome은 관측 가능한 receipt를 남겨야 한다");
      assert.equal(injectFound, false, "주입: receipt를 벗기면 정지가 silent가 돼 관측 단언이 not-found로 뒤집혀야 한다(안 바뀌면 헛단언)");
      report.push(`오라클③ 조용한질문 : 무주입->receipt FOUND(관측가능)=GREEN | 주입(receipt strip)->NOT FOUND(silent) (무주입 기대 'FOUND' 적용 시 RED) ⇒ 관측 단언 load-bearing`);
    } finally {
      cleanup(dir);
    }
  }
  ...
  // 적용 시 RED). (harnessDir 격리 교차소비는 S1이 handshake로 이미 커버.)
  {
    ...
    try {
      ...
      const clean = supervisor.runSupervisedAttempt(dirClean, "arm-nc4", SCOPE, makeTask(), { nowFn: NOW_OK, adapterFn: adapterClean });
      const inject = supervisor.runSupervisedAttempt(dirInject, "arm-nc4", SCOPE, makeTask({ lane: "review" }), { nowFn: NOW_OK, adapterFn: adapterInject });
      assert.equal(adapterClean.count(), 1, "무주입: 자기 lane task는 정확히 1회 소비된다");
      assert.equal(clean.outcome, "done");
      assert.equal(adapterInject.count(), 0, "주입: 타 lane task는 절대 소비되면 안 된다 -- 소비되면 교차소비가 실재");
      assert.equal(inject.phase, "own_consumption", "주입: 거부는 lane 가드(own_consumption) 때문이어야 한다 -- 우연한 다른 실패가 아니라");
      report.push(`오라클④ 타레인소비 : 무주입(lane=coder)->CONSUMED(spawns=1)=GREEN | 주입(lane=review)->REFUSED(own_consumption, spawns=0) (무주입 기대 'spawns=1' 적용 시 RED) ⇒ 소비경계 단언 load-bearing`);
    } finally {
      cleanup(dirClean);
      cleanup(dirInject);
    }
  ...
  // clean측(false->admit)을 더해 변수를 격리한다(review-2 ⑤ 처방: mutation으로 확인·강화).
  {
    ...
    try {
      ...
      const tampered = { ...raw.store, grant: { ...raw.store.grant, publish_allowed: true } };
      ...
      assert.equal(clean.spawnAllowed, true, "무주입(publish_allowed:false): 정상 grant는 admit된다");
      assert.equal(inject.spawnAllowed, false, "주입(publish_allowed:true): 변조된 사람 게이트는 거부돼야 한다 -- admit되면 게이트 우회 실재(안 바뀌면 헛단언)");
      ...
      assert.equal(inject.store?.disarm_cause, DISARM_CAUSE.STATE_CORRUPT, "주입 거부의 disarm_cause는 state_corrupt여야 한다 -- 손상 사유가 정확히 이 게이트를 추적함이 증명돼야 함");
      assert.ok(typeof inject.reason === "string" && inject.reason.includes("publish_allowed"), `주입 거부의 reason에 publish_allowed가 포함돼야 한다 -- got '${inject.reason}'`);
      report.push(`오라클⑤ 사람게이트 : 무주입(publish=false)->ADMITTED=GREEN | 주입(publish=true)->REFUSED(state_corrupt) (무주입 기대 'ADMITTED' 적용 시 RED) ⇒ 게이트 단언 load-bearing`);
    } finally {
      cleanup(dirClean);
      cleanup(dirInject);
    }
  ...
  console.log("[go-wait-pilot][negative-control] 각 오라클: 위반 주입 시 탐지(무주입 기대 적용 시 RED), 무주입 시 통과(GREEN) 대비 --");
  for (const line of report) console.log(`  ${line}`);
  assert.equal(report.length, 5, "5 오라클 negative control이 전부 실행돼야 한다");
});
