...
// HYK-270-wake-fire-2 (coder-task.md §2/§3, 검토 P1 수리): 부인절이
// "승인·판정" 낱말을 그대로 써서 그 자체가 금지어 검사의 오탐 원인이었다
// (검토 재현: `hits=[]`가 나온 진짜 이유는 검사 범위가 좁아서였지만, 범위를
// 넓히면 이 문장 자신이 오탐됐다). 뜻은 그대로 두고 낱말만 교체했다 --
// "승인"은 "허가"로, "판정"은 "결정"으로. 이제 WAKE_MESSAGE_FORBIDDEN_WORDS
// 그 어떤 항목도 이 문구 전체(부인절 포함)에 나타나지 않는다 -- 그래서
// 금지어 검사를 문구 "전체"로 되돌려도(부분 절제 없이) 이 문구는 초록이다.
