...
// (HYK-280 재실측, C:\Users\Administrator\.claude-team\projects\ ·
// ~/.claude/projects\ 아래 실재 디렉터리 이름 101+5개 전수 대조):
//
// ★이전 버전(경로 구분자·콜론 셋만 치환)의 거짓 NOT_STARTED 근거
// (ORCH 실측, 2026-09-23): 배달기가 파생한 폴더
// `...-모바일마크다운에디터-hyk304-linebreak-1`은 디스크에 존재하지
// 않는다 -- 실제 폴더는 `...------------hyk304-linebreak-1`(한글
// 10글자가 대시 10개로 치환돼 있다). 없는 폴더를 읽어 관측 0을 얻고
// 그것을 "아예 시작 못 함"(NOT_STARTED)으로 접었다 -- 오늘만 8회.
//
// ★실측 결론 -- 영숫자([a-zA-Z0-9]) 이외의 «모든» 문자를 각각 독립된
// `-` 한 글자로 치환한다(병합 없음, 코드 뒤섞기 없음):
// - 경로 구분자(`\`,`/`)·콜론(`:`)·점(`.`)은 전부 이 규칙의 부분집합일
//   뿐이다(별도 취급 아님) -- 실물 증거:
//   `hyk442-blocked-door-1--harness-tmp-hookprobe`(대시 2개 연속 =
//   `\` + `.`가 병합 없이 각각 치환된 것, 숨김 폴더
//   `.harness-tmp-hookprobe`의 선행 점).
// - 한글(BMP, U+AC00~U+D7A3 등)도 한 글자당 대시 한 개다(병합 없음)
//   -- 실물 증거 3건, 전수 일치:
//   `모바일마크다운에디터`(10자) -> 대시 10개,
//   `문서관리`(4자) -> 대시 4개, `통역`(2자) -> 대시 2개.
// - ⚠️정직 한계 -- 이 저장소·이 기계에서 관측 가능한 106개(101+5) 실물
//   폴더 중 서라게이트 쌍(이모지 등 BMP 밖 문자)을 쓴 예는 0건이었다.
//   그래서 "유니코드 한 글자 = 대시 몇 개"가 코드포인트 기준인지
//   UTF-16 코드유닛 기준인지는 한글만으로는 갈리지 않는다(둘 다
//   1글자=1코드유닛=1코드포인트라 구별이 안 됨). 이 구현은 `u` 플래그
//   없는 `[^a-zA-Z0-9]` -- UTF-16 코드유닛 기준(서라게이트 쌍이면 대시
//   2개)을 택했다. 실물 데이터 106건 전부와 일치하는 가장 단순한
//   가설이라 골랐을 뿐, 서라게이트 쌍 자체를 실측한 것은 아니다 -- 그런
//   실물이 나오면 이 가정을 재검증해야 한다.
export function deriveClaudeProjectDirName(absRepoRoot) {
  return String(absRepoRoot).replace(/[^a-zA-Z0-9]/g, "-");
}
