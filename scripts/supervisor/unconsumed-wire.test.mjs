...
// 시험들(async 콜백 + `await withTempDir(...)`)에서 그 콜백이 첫
// `await`에서 제어를 넘기는 순간 임시 폴더가 지워져, 늦게 도착하는
// 파일 접근이 ENOENT로 실패할 수 있었다(CI에서만 드러나는 비결정적
// 실패, HYK-460 5794334422 로그와 동형). `fn(dir)`의 반환값이
// thenable이면 정착(resolve/reject)된 뒤에만 정리하고, 아니면(이 파일의
// 압도적 다수인 동기 콜백) 기존처럼 즉시 정리한다 -- 동기 호출부(대다수,
// await 없이 호출)의 동작은 한 글자도 안 바뀐다(★naive async/await로
// 바꾸면 동기 콜백에서의 동기 throw가 미처리 프로미스 거부로 바뀌어
// 오히려 새 회귀를 만든다 -- 그래서 thenable 여부를 직접 검사한다).
