...
// 파일 접근이 ENOENT로 실패할 수 있었다. `fn(dir)`의 반환값이 thenable
// 이면 정착된 뒤에만 정리하고, 아니면(이 파일의 압도적 다수인 동기
// 콜백) 기존처럼 즉시 정리한다 -- 동기 호출부(대다수, await 없이 호출)
// 의 동작은 한 글자도 안 바뀐다(naive async/await면 동기 콜백에서의
// 동기 throw가 미처리 프로미스 거부로 바뀌어 새 회귀를 만든다).
