...
//
// ⛔⛔ HYK-394-guard-self-4 §2 Q2 (검토자 rejected 판정, 2026-08-30 정정):
// 아래 P1 anchor는 **계약(«배달 시점에 파일에 박아 둔 값»)이 아니다.**
// 이 헤더의 옛 문면("진짜 «박아 둔 값» 읽기")이 계약을 충족한 것처럼
// 읽혀 오해를 살 수 있어 정정한다 -- dispatch-gate-decision.mjs 프로덕션
// 주석(findLatestReceiptMatch 헤더의 "⛔⛔ HYK-394-test-leak-3 §2 Q3"
// 블록)은 이미 정확했다: currentBinding과 candidate 양쪽 dispatchId가
// 여전히 같은 (role, harnessTaskLabel) 조회 함수에서 파생되므로, 다른
// 5개 결속 필드가 이미 일치하는 한 dispatchId는 구조적으로 redundant
// 하다. **배달 시점 보존(보존 사본에 dispatch_id를 실제로 굽는 것)은
// 이 저장소 범위에서 미결선이며, 완성은 HYK-396 예정이다.** 아래는
// anchor가 «판정 시점 조회를 안정화»한다는 것만 서술한다 -- 그 이상을
// "박아 둔 값"이라고 주장하지 않는다.
//
...
// 실제로 닫는, 독립적으로 안전한 개선이다(단, ⛔위 정정 그대로 이것이
// "배달 시점 결속" 계약 자체를 만족했다는 뜻은 아니다). P2(1R이 안전하지
// 않아 되돌렸던 "가장 높은 번호 사본 하나만 보지 않고 사본 중 하나라도
// 일치하면 소비 인정" 완화)는 이 라운드에서 dispatch_id anchor와 함께
// 다시 넣어 직접 실행해 봤지만(시험 (b)), ★여전히 뚫렸다 -- 그래서 Q8
// 지시 원문("통과시키면 넣지 마라") 그대로 다시 반려했다. 아래 두 시험은
// 그 최종(P2 반려 후) 코드의 실제 동작을 단언한다.
//
...
// 언제인지"와 무관하게 항상 같은 답을 낸다는 뜻일 뿐, ⛔"배달 시점에
// 박아 둔 값"이라는 계약 자체를 만족한다는 뜻이 아니다(위 정정 참조 --
// 조회가 판정 시점에 다시 일어난다는 사실 자체는 바뀌지 않았다, 다만
// 그 조회가 «안정적으로 같은 값을 낸다»는 것만 달라졌다).
//
