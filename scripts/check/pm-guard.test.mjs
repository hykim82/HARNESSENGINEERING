...
const SCRATCHPAD = "C:/Users/ADMINI~1/AppData/Local/Temp/claude/some-project/scratch.md";

test("(1) PM writing inside a repo is blocked", () => {
  const result = checkPmGuard({
    role: "PM",
    toolName: "Write",
    filePath: "C:/Users/Administrator/Documents/HARNESSENGINEERING/.harness/coder-task.md",
  });
  assert.equal(result.ok, false);
});

test("(2) PM writing the control room is allowed", () => {
  const result = checkPmGuard({ role: "PM", toolName: "Edit", filePath: CONTROL_ROOM });
  assert.equal(result.ok, true);
});

test("(3) PM writing the scratchpad is allowed", () => {
  const result = checkPmGuard({ role: "PM", toolName: "Write", filePath: SCRATCHPAD });
  assert.equal(result.ok, true);
});
...
test("(4) non-PM role is not regulated by pm-guard", () => {
  const result = checkPmGuard({
    role: "CODER",
    toolName: "Write",
    filePath: "C:/Users/Administrator/Documents/HARNESSENGINEERING/scripts/check/foo.mjs",
  });
  assert.equal(result.ok, true);
});

test("(5) unset role is not regulated by pm-guard", () => {
  const result = checkPmGuard({ role: undefined, toolName: "Write", filePath: "C:/anywhere/file.md" });
  assert.equal(result.ok, true);
});

test("(6) PM calling a Linear write MCP tool (save_) is blocked", () => {
  const result = checkPmGuard({ role: "PM", toolName: "mcp__linear-server__save_issue", filePath: undefined });
  assert.equal(result.ok, false);
});

test("(7) PM calling a Linear write MCP tool (create_) is blocked", () => {
  const result = checkPmGuard({ role: "PM", toolName: "mcp__linear-server__create_issue", filePath: undefined });
  assert.equal(result.ok, false);
});

test("(8) PM calling a Linear write MCP tool (delete_) is blocked", () => {
  const result = checkPmGuard({ role: "PM", toolName: "mcp__linear-server__delete_issue", filePath: undefined });
  assert.equal(result.ok, false);
});

test("(9) PM calling a Linear read MCP tool is not blocked", () => {
  const result = checkPmGuard({ role: "PM", toolName: "mcp__linear-server__list_issues", filePath: undefined });
  assert.equal(result.ok, true);
});

test("(10) PM calling a non-write tool (Read) is not regulated", () => {
  const result = checkPmGuard({ role: "PM", toolName: "Read", filePath: "C:/anywhere/file.md" });
  assert.equal(result.ok, true);
});
