...
// 우회 불가"(§C-5 비타협).
//
// 옆문 2문 검사(§C-4가 요구): ⑴감사에서 보이는가 = 예(override 파일
// 자체가 git 워크트리 안 파일이라 커밋 이력에 남는다 -- "통지 축") ⑵
// 없으면 정당 작업이 막히는가 = 예(런처가 죽으면 복구 좌석을 손으로
// 띄워야 하는데, 1단계는 "거부"가 아니라 "경고"이므로 애초에 막히지도
// 않지만, 2단계(거부)가 열릴 때를 대비해 지금 판정 경로를 미리 검증해
// 둔다) -> 설계된 문.
//
