# HYK-419 — "은퇴 기록을 누가 쓰는가": 작성 권한 경계 + 자동화 코어 설계 (4R 수리)
...
## 4R 수리 요약 (검토 P1-1 재반려 반영 — 완료조건 §⑾ⓗ 미충족, 스펙 문면 결함으로 원인 재분류됨)

3R의 realpath 수리 자체는 유효했다(링크·정션·중첩 링크·`./..` 혼합·UNC·
긴 경로·디렉터리 링크 전부 닫힘·과차단 0·되돌림 변이 9/9 RED 실측 확인 —
그 성과는 이 라운드에서 되돌리지 않는다). 검토가 새로 지적한 것은
`scripts/check/retirement-auto-author-core.test.mjs`의 정상 경로 fixture
`PAST_RECORDED_AT = "2020-01-01 00:00:00 KST"`가 **절대 시각 상수**라는
점이다 — 지금 실행에서는 우연히 과거라 통과할 뿐, "과거 시각이면 통과
한다"는 요구를 실제로 시험한 증거가 아니다(그 상수는 2R 커밋에서
들어왔고, 이 문턱 자체는 3R 검토 지시서에서 처음 등장했다 — ORCH 문면
결함이 근본 원인이라는 사실은 그대로 남기되, 요구 자체는 정당하므로
고친다). 이 라운드는 그 한 상수를 `Date.now()`에서 매 호출마다 파생되는
상대값(30분 전)으로 바꾸는 **최소 수리 하나만** 한다(§4·§6·§7).
...
`retirement-auto-author-core.test.mjs`에 정확히 **10건**(2R의 8건 + 3R의
realpath 재확인 관문 되돌림 변이 1건 + 4R의 fixture 상대화 되돌림 변이
1건 = 10), 문서(이 절)의 숫자와 일치한다. 1~9건은 각각 원본 소스 파일
(`retirement-auto-author-core.mjs`)을 메모리에서 읽어 대상 문자열이 정확히
...
동일 규율). 10번째(★4R)는 대상이 다르다 — 이 시험 파일 «자신»의 fixture
헬퍼(`pastRecordedAt`)를 대상으로 하는 자기참조적 변이라, 동적 임포트가
아니라 "이 파일 자신의 소스 텍스트를 메모리에서 변형해 별도 검사기
함수에 통과시킨다"는 형태를 쓴다(아래 항목 10, 원복 증명도 이 파일
자신을 재확인하는 형태로 동일하게 적용한다).
...
9. 후속 이름표 문법 확인 관문(`checkSuccessorLabelGrammar` 호출) 제거
   ...
   까지 새어 나감.
10. ★4R 신설 — `pastRecordedAt()`의 본문을 `Date.now()` 기반 상대값
    계산에서 `"2020-01-01 00:00:00 KST"` 절대 시각 상수로 되돌림 →
    이 시험 파일이 자체적으로 갖춘 정적 검사기(`fixtureUsesAbsoluteTimestamp`,
    "pastRecordedAt() 본문에 `Date.now()`가 있는가"만 본다)가 위반으로
    잡는다. GREEN 대조군("★4R 정적 검사")이 현재 소스에서는 검사기가
    위반 없음을 먼저 확인해 두므로, 이 변이가 실제로 검사기를 발화
    시켰다는 것이 공허하지 않다.
...
- **이 코어의 소스 파일 자체를 몰래 패치하는 공격**은 못 막는다(예: §6의
  10개 되돌림 변이 중 하나가 실제로 소스에 반영되는 것) — "부재 자체가
...
### ★4R 정정 — 시험 fixture 자체의 정직성 (코어 코드가 아니라 시험 위생)

4R이 고친 것은 **코어(`retirement-auto-author-core.mjs`) 자신의 판정
로직이 아니라, 그 판정 로직을 "정상 경로" 표본으로 검증하는 시험 파일의
fixture 값**이다 — `PAST_RECORDED_AT`이 절대 시각 상수였던 것을 `Date.now()`
에서 매 호출마다 파생되는 상대값(30분 전)으로 바꿨다. 이건 코어의 위조
표면을 하나도 넓히거나 좁히지 않는다 — "정상 경로가 실제로 통과하는가"
라는 시험 자체가 시간이 지나도(2020년이 더 이상 "명백한 과거"가 아니게
될 먼 미래에도 걱정할 필요 없이) 계속 유효한 증거로 남게 하는, 순수하게
시험 위생 문제였다. 그래서 이 정정은 §5(위조 표면 열거)·§4(자동 작성
코어)의 어떤 항목도 바꾸지 않는다 — 오직 §6(되돌림 변이 개수)만 10건으로
늘었다.

**그래도 정직하게 남기는 것**: 30분이라는 구체적인 간격 선택은
"9시간(540분) ±10분 대역과 충분히 멀다"는 근거 하나로 정당화했다 —
이 상수 자체가 미래의 어느 시점에 다시 문제가 될 수 있는 다른 대역
(예: 이 저장소에 새로운 시간대-오라벨 휴리스틱이 다른 오프셋으로
추가되는 경우)까지 예견해서 피한 것은 아니다. 지금 존재하는 유일한
알려진 위험 대역(`TZ_MISLABEL_TOLERANCE_MS`, 9시간 ±10분)만 명시적으로
피했다는 뜻이다.
