...
## 적용 상태: **제안(PROPOSED)** — 아직 관제실에 적용되지 않았다

이 문서는 HYK-256 문서(`HYK-256-dispatch-worker-receipt-path.md`)와 달리 **아직 APPLIED가 아니다**. 이 워크트리는 관제실(`D:\문서관리\하네스-관제실\`)에 쓰기 0(coder-task.md §0 비타협2) — ORCH가 아래 «교체 문면»을 그대로 복사해 적용한 뒤, 이 문서 머리를 HYK-256 문서와 같은 모양의 «적용 상태» 표로 갱신해야 한다.
