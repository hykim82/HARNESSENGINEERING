...
## 적용 상태: **WIRED** — ORCH가 6단계 전례 절차(문면 → 독립 검토 → 기계 추출 → 지문·백업·파싱 검사 → 관제실 git 커밋 → 발동 관측)를 실제로 집행했다(HYK-328-receipt-name-1 동봉 갱신, ORCH가 오늘 19:0x에 직접 재측정한 실측값 그대로).

### 적용 증거

- **적용 시각**: 2026-08-20 17:04 KST · **관제실 커밋**: `a1e8e46`(`fix(dispatch-worker): HYK-286 …`)
- **지문 대조**: 적용 전 SHA-256 `29fb025f23dbf8ae14f9adf81305de20975996ab0312ef1a352f67413aa707e6`
  → 적용 후 SHA-256 `c366edd32436942745321ff66c47d532fbbb216eae631e436289c95495bbaea0`.
- **백업**: `dispatch-worker.ps1.bak-hyk286-20260820-170429`(관제실 git 커밋
  전 되돌릴 수 있는 사본, §6 절차 2단계 그대로).
- **발동 관측**: codex 세션 수집 착수확인 exit 2 → 0(§0 실측 문구
  "empty session_meta line"이 더 이상 배달을 조기 실패시키지 않는다).

★**저장소 안에서 스스로 확인 가능한 대조**(HYK-328-receipt-name-1이 직접
재계산해 위 값과 일치를 확인했다 — 이 문서가 헛말이 아니라는 근거):

- 적용 「전」SHA-256은
  `scripts/check/fixtures/dispatch-worker-snapshot-2026-08-20-hyk327-applied.ps1.txt`
  의 SHA-256과 **동일**(재계산 결과: `29fb025f23dbf8ae14f9adf81305de20975996ab0312ef1a352f67413aa707e6`, 일치).
- 적용 「후」SHA-256은
  `scripts/check/fixtures/dispatch-worker-snapshot-2026-08-20-hyk286-applied.ps1.txt`
  의 SHA-256과 **동일**(재계산 결과: `c366edd32436942745321ff66c47d532fbbb216eae631e436289c95495bbaea0`, 일치).

이 증거로 §6의 6단계 전례 절차(문면 → 독립 검토 → 기계 추출 → 지문·백업·
파싱 검사 → 관제실 git 커밋 → 발동 관측) 전부가 완료됐음을 확인한다.
⛔이 CODER 라운드(HYK-328-receipt-name-1) 자신은 관제실 파일을 고치지
