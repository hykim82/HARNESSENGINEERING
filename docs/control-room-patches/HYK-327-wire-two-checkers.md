...
## 적용 상태: **PROPOSED** — ⛔이 라운드(CODER, HYK-327-wire-2, 재작업)도 이 문서를 쓸 뿐 관제실 파일을 고치지 않는다(coder-task.md §0 비타협2 "관제실 쓰기 0"). 적용은 ORCH가 6단계 전례 절차(문면 → 독립 검토 → 기계 추출 → 지문·백업·파싱 검사 → 관제실 git 커밋 → 발동 관측)로 집행한다.
