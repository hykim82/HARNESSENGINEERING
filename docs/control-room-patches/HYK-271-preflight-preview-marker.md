...
- ★**갱신(HYK-271-marker-catalog-3, 2R 수리)**: 2R widen 이 `Enter to select` 를 **단독 부분일치**로 추가했으나, 검토(P1-1)가 정상 문장을 MODAL로 오판하는 위양성을 실증했고, ORCH 는 별도로 **이 파일 자신을 편집 중인 좌석 화면**(§1 인용)도 오판됨을 관측했다. 이번 수리는 그 단독 마커를 **두 메커니즘으로 교체**했다:
  1. `MODAL_TAIL_COMBO`(`dispatch-worker-modal-check.mjs`) — `Enter to select`가 동반 문구(`to navigate`/`Esc to cancel`/`Esc to back`) 중 하나와 **정규화 후 15자 이내**로 근접할 때만 잡는다. 실측 근접 거리: 창립 표본 3자, 오늘 표본 7자(둘 다 여유 있게 15자 문턱 안).
  2. `stripSelfCatalogSourceQuotes`(같은 파일) — §1 이 인용한 화면은 `Enter to select` 뿐 아니라 **원래 1R 마커인 `"Allow command?",`도 JS 소스 리터럴 형태로 포함**하고 있어, 자기편집 오판이 새 마커에만 국한된 문제가 아님이 이번에 추가로 드러났다. 카탈로그가 소유한 모든 문자열에 대해 `"<문자열>",` 정확한 모양을 매칭 전에 제거한다.
  - **위양성 0 측정**: 두 실사고 표본(MODAL 유지) · 살아 있는 좌석 14개(NON_MODAL 유지) · 새 적대 표본 3개(평범한 메뉴 사용법 문장 · 문서/주석 언급 · §1 자기편집 화면 원문, 전부 NON_MODAL) — `scripts/relay/hyk271-marker-catalog-real-corpus.test.mjs`.
  - **잔여 위험(숨기지 않음)**: (a) 실제 꼬리줄 전체를 " · " 간격 그대로 인용하는 문서가 있다면 여전히 MODAL 로 잡힌다(콤보 메커니즘의 한계, §3-4 참조). (b) `stripSelfCatalogSourceQuotes`는 정확히 `"<문자열>",` 모양만 제거한다 — 작은따옴표·백틱·다른 후행 문자(예: 배열 마지막 항목의 후행 콤마 없음)로 렌더된 인용은 제거되지 않아 여전히 자기편집 오판을 일으킬 수 있다.
...
- ★**갱신(HYK-271-marker-catalog-3, 2R 수리)**: 2R widen 의 단독 `Enter to select` 마커는 «위양성 0을 실물 14개 corpus로 측정»했다고 주장했지만, 그 corpus에는 «평범한 문장이 그 문구를 언급하는 경우»와 «이 파일 자신을 편집 중인 화면»이 빠져 있었다 — 검토가 전자를, ORCH 관측이 후자를 잡아냈다. 이번 수리(근접 콤보 + 자기카탈로그 인용 제거)는 이 두 가지에 대해 위양성 0을 측정했지만(§1-b 갱신), **그래도 뚫리는 모양**이 남는다: (a) 실제 메뉴 꼬리줄을 " · " 간격까지 그대로 베낀 문서·주석은 여전히 MODAL 로 잡힌다(콤보가 «근접성»만 보고 «렌더링 출처»는 모른다), (b) 자기카탈로그 인용 제거는 정확히 `"<문자열>",` 모양(큰따옴표+콤마)만 없앤다 — 작은따옴표·백틱·후행 콤마 없는 렌더는 그대로 남는다. 이 두 잔여 위험은 «닫혔다»고 주장하지 않는다.
