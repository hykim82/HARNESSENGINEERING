...
- ★**갱신(HYK-271-marker-catalog-1, 2R widen)**: 위 문단(1R)이 쓰였을 때는 사실이었지만 지금은 낡았다 — 그때는 "실제 사고의 인용이 0건"이었고, 지금은 **2건**이다(창립 표본 = HYK-271 이슈 본문 자체, 오늘 표본 = 2026-09-01 HYK-415 워커 좌석 20분 정지 관측, 둘 다 `scripts/relay/hyk271-marker-catalog-real-corpus.test.mjs`에 원문 그대로 박혀 있다). 이 2건에 대해서는 **"카탈로그가 실제 사고 문구와 일치하는가"가 이제 «측정»이다** — `Enter to select` 마커가 둘 다 잡는다(위양성 0, 살아 있는 좌석 14개 실물 preview 대조).
  - **아직 검증 안 된 것**: 최초 4개 마커(명령 승인창 문구)는 여전히 **공개적으로 알려진 형태를 손으로 지은 것**이지 이 저장소가 겪은 실제 사고의 인용이 아니다(1R 그대로, 갱신 없음). 그리고 이번에 잡은 2건도 "선택 메뉴" 계열 중 **이 두 화면**일 뿐이다 — 다른 언어(전부 영어 꼬리줄 없는 메뉴)·다른 모양의 선택 메뉴까지 이 마커가 잡는지는 미검증이다(§3 참조).
