...
- 공통: `relay/pm.md` = 상단 `task_id:` 에코 + 요약(≤5줄) + 산출물 경로 + 마지막 줄 `>>> DONE: PM @ 시각 KST` -- 이 줄은 `node scripts/relay/finalize-done.mjs PM .harness`를 실행해 만든다(손기입 금지). ⛔**쓴 뒤에는 이 결과 파일을 다시 고치지 마라** — 소비 영수증이 그 시점의 지문을 기록하므로, 서식만 고쳐도 이미 소비된 라운드가 영구 «미소비»로 오판된다(HYK-244 실사고).
