...
This is a capture-assist tool for HYK-96 Scope C. It lowers the friction of
keeping `PROJECT-CONTEXT.md` current; it does not replace human judgment
about what belongs there. See "Honest limits" below before relying on it.

## Procedure

1. **Resolve the card path.** Do not assume `.harness/PROJECT-CONTEXT.md` —
   the active card can live outside this repository. Resolve in this exact
   order, matching `scripts/check/context-inject.mjs`'s own precedence
   (`resolveContextPath`):
   1. **(a)** Read `.claude/settings.local.json`. Find the `SessionStart` or
      ...
      In this repository, as of this writing, that resolves to
      `D:/문서관리/하네스-관제실/PROJECT-CONTEXT.md` (the control room card,
      outside this repo) — but re-read the file each time rather than
      trusting this note, since the wiring is per-clone and can change.
   2. **(b)** If no `--context` argument is found, use the
      `HARNESS_CONTEXT_PATH` environment variable if set.
   3. **(c)** Otherwise, default to `<repo-root>/.harness/PROJECT-CONTEXT.md`.
...
3. **Scan the conversation for durable facts.** Look across the session for
   ...
   - **Hard constraints** — short, imperative, non-negotiable rules whose
     violation would be a real incident (not a style preference).
...
7. **Self-check the result.** Run, against the resolved card path from step 1:
   ...
   ```sh
   node scripts/check/context-inject.mjs --mode user-prompt-submit --context <resolved-path>
   ```
...
- **Not a hard gate.** This skill only runs when explicitly invoked
  ...
  future work (HYK-96 Scope B), not part of this skill.
